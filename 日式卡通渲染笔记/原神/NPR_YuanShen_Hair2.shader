Shader "NPR/NPR_YuanShen_Hair2"
{
    Properties
    {
        ...
        [Space(30)]
        _LightThreshold("_LightThreshold",Range(-1,1))=0.996
    ...
    SubShader
    {
        ...
        #define UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input)
            ...
            float _LightThreshold, _RampOffset, _BrightIntensity, _DarkIntensity, _RampIntensity,_CharacterIntensity;
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                MetalMap = step(_MetalMapV,MetalMap)*MetalMap*_MetalMapIntensity;
                
                FinalColor.rgb = BaseMap.rgb;

                float SpecularLayerMask = LightMap.r; //SpecularExpMask
                float SpecularIntensityMask = LightMap.b; //SpecularIntensityMask
                float ShadowMask = LightMap.g; //ShadowMask
                float LayerMask = LightMap.a; //LayerMask
                float RampOffsetMask = VertexColor.g; //Ramp偏移值,值越大的区域 越容易"感光"(在一个特定的角度，偏移光照明暗)

                // float RampPixelY = _RampMap_TexelSize.y;
                ...
                if (LayerMask >= 0.95 && LayerMask <= 1.0)
                {
                    ...
                    // return BaseMap*_CharacterIntensity;
                }
                ...
                ShadowMask = 1 - smoothstep(saturate(ShadowMask), 0.2, 0.6); //平滑ShadowMask,减弱锯齿

                //为了将ShadowMask区域常暗显示
                float3 ramp = tex2D(_RampMap, saturate(float2(halfLambert * lerp(0.5, 1.0, ShadowMask), PixelInRamp)));
                float3 BaseMapShadowed = lerp(BaseMap * ramp, BaseMap, ShadowMask);
                BaseMapShadowed = lerp(BaseMap, BaseMapShadowed, _ShadowRampLerp);
                float IsBrightSide = ShadowMask * step(_LightThreshold, halfLambert);
                ...
                /*
                 //裁边高光 (高光在暗部消失)
                // if (SpecularLayer > 100 && SpecularLayer < 200)
                // {
                ...
                */
                ...
                float3 HairSpecular = 0;
                 ...
                 //头发高光
                if (SpecularLayer >= 200 && SpecularLayer<250)
                {
                ...
                FinalColor.rgb = Diffuse + Specular +StepSpecular +StepSpecular2 + HairSpecular;
